...
class AdminOrdersTabs extends React.Component {
  ...
  renderDataTableOrders1(){
      var arrJSX = this.state.orders1.map((item)=>{
          return(
          <AdminRenderOrders key={item.user_id} transaction_id={item.id} ordernumber={item.ordernumber} user_id={item.user_id} proof={item.proof} name={item.name} date={item.date} shipping={item.shipping} time={item.time} subtotal={item.subtotal} target_bank={item.target_bank}
